...
function remove(userId) {
  ...
  if(gUsers[idx].isAdmin) return Promise.reject('Cannot delete admin')
  if(gUsers[idx].bugs.length > 0) return Promise.reject('Cannot delete user with bugs')
...
//UPDATE
function saveBugInUser(userId,bug){
    console.log('here');
    const idx = gUsers.findIndex(currUser => currUser._id === userId)
    gUsers[idx].bugs.push(bug)
